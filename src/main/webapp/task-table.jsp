			...
			<div class="container-fluid">
				<div class="row bg-title">
					...
					<div class="col-lg-9 col-sm-8 col-md-8 col-xs-12 text-right">
						<a href="task-add" class="btn btn-sm btn-success">Add</a>
					</div>
					<!-- /.col-lg-12 -->
				</div>
				<!-- /row -->
						...
						<div class="white-box">
							<div class="table-responsive">
								<table class="table" id="example">
									...
									<tbody>
										<c:forEach var="dataTask" items="${listTask}">
											<tr>
												<td>${dataTask.id}</td>
												<td>${dataTask.name}</td>
												<td>${dataTask.project.name}</td>
												<td>${dataTask.user.firstname}
													${dataTask.user.lastname}</td>
												<td>${dataTask.start_date}</td>
												<td>${dataTask.end_date}</td>
												<td>${dataTask.status.name}</td>
												<td><a
													href="/helloservlet/task-modify?id=${dataTask.id}"
													class="btn btn-sm btn-primary">Modify</a> <a
													href="/helloservlet/task-delete?id=${dataTask.id}"
